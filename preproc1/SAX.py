...
class SAX():
    def process(self, data, param):
        ...
        size=data.shape
        N=size[0]/int(window)
        SaxData=np.zeros((N,size[1])).astype('str')
        print type(SaxData[0])
        for c in range(0,size[1]):
            Temp=data[:,c]
            SaxDataTemp = to_sax(to_paa(normalize(Temp),Temp.__len__()/(int(window))),int(sax_vocab))
            SaxData[:,c]=SaxDataTemp

        p=SaxDataTemp[0].split(',')
        print type(p)
        result=pd.DataFrame(SaxData)



        #return vocabToCoordinates(data.__len__()/(int(window)),data.__len__()/(int(window)),sax[0],int(sax_vocab))
        return result
